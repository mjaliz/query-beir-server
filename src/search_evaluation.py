        ...
        for result in self.results:
            if result.false_positives:
                ...
                for fp_doc_id in result.false_positives:
                    ...
                    # Extract relevant queries information from payload
                    relevant_queries = payload.get("relevant_queries", [])
                    fp_query_ids = []
                    fp_query_texts = []
                    for query_info in relevant_queries:
                        if isinstance(query_info, dict):
                            fp_query_ids.append(query_info.get("query_id", ""))
                            fp_query_texts.append(query_info.get("query_text", ""))
                        else:
                            # Handle old format where it might just be query_id strings
                            fp_query_ids.append(query_info)
                            fp_query_texts.append("")
                    ...
                    record = {
                        ...
                        "corpus_id": corpus_id,
                        "fp_query_ids": fp_query_ids,
                        "fp_query_texts": fp_query_texts,
                        "score": score,
